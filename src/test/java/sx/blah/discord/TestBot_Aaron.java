...
public class TestBot_Aaron {
	...
	public static void main(String... args) {
		try {
			DiscordClient.get().login("aaronawest@gmail.com", "TidiBot");
			...
			DiscordClient.get().getDispatcher().registerListener(new IListener<MessageReceivedEvent>() {
				@Override public void receive(MessageReceivedEvent messageReceivedEvent) {
					...
					if (m.getContent().startsWith(".meme")
                    	...
                    	new MessageBuilder().appendContent("TEST MESSAGE PLEASE IGNORE").withChannel(messageReceivedEvent.getMessage().getChannel())
                    		.build();
                    } else if (m.getContent().startsWith(".hello")) {
                    	new MessageBuilder().appendContent("Hello world!").withChannel(messageReceivedEvent.getMessage().getChannel())
                    		.build();
                    }
				}
			});
